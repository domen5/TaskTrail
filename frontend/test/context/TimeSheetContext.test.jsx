...
describe('TimeSheetContext', () => {
    ...
    describe('getMonthData', () => {
        it('fetches and processes month data correctly', async () => {
            ...
            let result;
            await act(async () => {
                result = await timeSheetActions.getMonthData(2025, 1); // February (0-based)
            });
            
            expect(api.getMonthWorkedHoursApiCall).toHaveBeenCalledWith(2025, 1);
            expect(Object.keys(result)).toHaveLength(1);
            
            // Verify the data is processed with timestamp keys
            const expectedTimestamp = new Date('2025-02-26').setHours(0, 0, 0, 0);
        ...
        it('handles empty month data', async () => {
            ...
            let result;
            await act(async () => {
                result = await timeSheetActions.getMonthData(2025, 1);
            });
    ...
    describe('updateDayData', () => {
        it('creates new worked hours entry successfully', async () => {
            ...
            const dayData = timeSheetActions.getDayData(new Date('2025-02-26'));
    ...
    describe('Month Locking', () => {
        it('locks month successfully', async () => {
            ...
            await act(async () => {
                await timeSheetActions.lockMonth(2025, 1);
            });
            
            expect(api.lockMonthApiCall).toHaveBeenCalledWith(2025, 1, true);
            expect(timeSheetActions.isMonthLocked(2025, 1)).toBe(true);
        });
        ...
        it('unlocks month successfully', async () => {
            ...
            await act(async () => {
                await timeSheetActions.unlockMonth(2025, 1);
            });
            
            expect(api.lockMonthApiCall).toHaveBeenCalledWith(2025, 1, false);
            expect(timeSheetActions.isMonthLocked(2025, 1)).toBe(false);
        });
        ...
        it('checks locked status correctly', async () => {
            ...
            await act(async () => {
                await timeSheetActions.checkAndSetLockedMonth(2025, 1);
            });
            
            expect(api.verifyLockedMonthApiCall).toHaveBeenCalledWith(2025, 1);
            expect(timeSheetActions.isMonthLocked(2025, 1)).toBe(true);
        });
        ...
        it('handles lock failure and reverts state', async () => {
            ...
            await act(async () => {
                await expect(timeSheetActions.lockMonth(2025, 1)).rejects.toThrow('Lock failed');
            });
